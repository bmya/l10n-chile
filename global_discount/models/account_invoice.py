    ...
    def _compute_base_amount(self):
        ...
        for tax in self:
            ...
            if tax.invoice_id.global_discount_type in ['percent']:
                tax.base = round(
                    tax.base * (
                        1 - ((tax.invoice_id.global_discount / 100.0) or 0.0)))
            else:
...
class GlobalDiscount(models.Model):
    ...
    # global_discount = fields.Many2one(
    # 'discounts', string="Seleccione Descuento Global")
    global_discount = fields.Float(
        string="Descuento Global", default=0.00, readonly=True,
        states={'draft': [('readonly', False)]})
    global_discount_type = fields.Selection(
        [('amount', 'Monto'), ('percent', 'Porcentaje')],
        string="Tipo de descuento", readonly=True,
        states={'draft': [('readonly', False)]})
    global_discount_detail = fields.Char(
        string="Razón del descuento", readonly=True,
        states={'draft': [('readonly', False)]})
    amount_untaxed_global_discount = fields.Float(
        string="Global Discount Amount", store=True, default=0.00,
        ...
        if self.global_discount > 0:
            ...
            for id, t in taxes.iteritems():
                ...
                if tax.amount > 0 and discount > 0:
                    t['amount'] = (
                        t['amount'] * (1 - ((discount / 100.0) or 0.0)))
                tax_grouped[id] = t
        return tax_grouped
        ...
        for inv in self:
            if inv.global_discount > 0  and inv.global_discount_type:
                ...
                amount_untaxed = round(
                    sum(line.price_subtotal for line in inv.invoice_line_ids))
                afecto = 0
                ...
                if inv.currency_id and inv.currency_id != inv.company_id.\
                        currency_id:
                    amount_total_company_signed = inv.currency_id.compute(
                        amount_total, inv.company_id.currency_id)
                    amount_untaxed_signed = inv.currency_id.compute(
                        amount_untaxed, inv.company_id.currency_id)
                sign = inv.type in ['in_refund', 'out_refund'] and -1 or 1
                inv.amount_total_company_signed = amount_total_company_signed *\
                                                  sign
    ...
    def finalize_invoice_move_lines(self, move_lines):
        if not self.global_discount > 0:
            return super(GlobalDiscount, self).finalize_invoice_move_lines(
                move_lines)
        new_lines = []
        discount = self.global_discount
                ...
                if line[2]['tax_ids']:
                    for t in line[2]['tax_ids']:
                        ...
                        if imp.amount > 0  and line[2]['debit'] > 0:
                            ...
                            line[2]['credit'] -= int(
                                round((line[2]['credit'] * (discount /100))))
                            total += line[2]['credit']
                        elif line[2]['debit'] > 0:
                            total += line[2]['debit']
                        else:
        ...
        new_lines = super(GlobalDiscount, self).finalize_invoice_move_lines(
            new_lines)
        return new_lines
